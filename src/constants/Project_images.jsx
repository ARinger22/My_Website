const images = {
    machine: require('../Project_Images/Machine_Learning.png'),
    commercial : require('../Project_Images/Amazon_app.png'),   
    gui: require('../Project_Images/GUI.png'),   
    cycle: require('../Project_Images/cycle.png'),   
    automotive: require('../assets/automotive.png'),   
    blockchain: require('../Project_Images/blockchain.png'),   
    note_bot : require('../Project_Images/Note_Bot.jpeg'),
    xv6 : require('../Project_Images/xv6.jpeg'),
    QLearning : require('../Project_Images/Q-Learning.png'),
    steam: require('../Project_Images/steam.png'),
};

export default images;